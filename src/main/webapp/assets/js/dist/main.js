...
}, 700); // Handle animation

handleAppearAnimation(); // Select all text when active
...
var COLOR_PRIMARY = '#5f4d93';
var COLOR_RED = '#e37682';
$("#login-form").handle("submit", function (e) {
  var username = $('#username').value.trim().toUpperCase();
  ...
  _sweetalert["default"].fire({
    ...
    preConfirm: function preConfirm(method) {
      var axiosData = {
        method: method,
        url: 'schedule',
        data: {
      ...
      if (method === "get") {
        axiosData = {
          method: method,
          url: "schedule?username=".concat(username)
        };
      }

      return (0, _axios["default"])(axiosData).then(function (response) {
        return response;
...
function $(selector) {
  var selectorString = selector.substr(1, selector.length - 1);
  return selector[0] === "#" ? document.getElementById(selectorString) : selector[0] === "." ? document.getElementsByClassName(selectorString) : document.getElementsByTagName(selector);
} // Query Selector all
